...
2. Remove all audio streams except for the French one (note: you must map the video and subtitle streams too or they'll be excluded)
...
2. Untar it and copy the files to /usr/local/bin
...
2. Join the files using the concat demuxer
...
2. Set the default audio stream. For example, to set the third audio stream as default and remove default from the first audio stream (stream numbering starts at 0):
...
2. *View* > *Download Subtitles* > *Search by hash*

3. If any subtitles are found, they'll be listed in the bottom of the window. Select one of the found subtitles > *Download selection*

4. Edit the downloaded SRT file and remove the first and last subtitles
...
2. Copy the video and audio streams as well as the selected subtitle stream
...
2. Then, re-encode the video using Handbrake, burning the subtitles in
